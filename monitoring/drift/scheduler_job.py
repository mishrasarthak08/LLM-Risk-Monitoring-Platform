...
def fetch_rolling_window_data(session, feature_name: str, metric: str, hours: int = 24):
    ...
    if metric == "output_length":
        ...
        return np.array([1 if t[0] is not None else 0 for t in traces]) if traces else np.array([])
...
def write_drift_event(session, feature_name, metric, statistic_value, threshold, severity, sample_size):
    event = DriftEvent(
        id=uuid.uuid4(),
...
def evaluate_drift(feature_name: str):
    ...
    try:
        ...
        cur_length = fetch_rolling_window_data(session, feature_name, "output_length")
        if ref_length is not None and len(cur_length) > 0:
            ...
            if ks_res["p_value"] < threshold:
                write_drift_event(session, feature_name, "output_length_ks", ks_res["p_value"], threshold, "warning", len(cur_length))

        # 2. Refusal Rate Drift (Two-Proportion Z-Test)
        ref_refusals = get_active_reference(feature_name, "refusal_rate")
        cur_refusals = fetch_rolling_window_data(session, feature_name, "refusal_rate")
        if ref_refusals is not None and len(cur_refusals) > 0:
            # Handle if reference was stored as a dict or array
            ref_sum = ref_refusals.get("refusals", 0) if isinstance(ref_refusals, dict) else sum(ref_refusals)
            ref_len = ref_refusals.get("total", 1) if isinstance(ref_refusals, dict) else len(ref_refusals)
            
            z_res = refusal_rate_shift(int(ref_sum), ref_len, int(cur_refusals.sum()), len(cur_refusals))
            threshold = thresholds.get("refusal_rate_z_p_value", 0.05)
            if z_res["p_value"] < threshold:
                write_drift_event(session, feature_name, "refusal_rate_z", z_res["p_value"], threshold, "critical", len(cur_refusals))

        # 3. Judge Score Distribution Drift (PSI)
        ref_scores = get_active_reference(feature_name, "judge_score_fa")
        cur_scores = fetch_rolling_window_data(session, feature_name, "judge_score_fa")
        if ref_scores is not None and len(cur_scores) > 0:
            ...
            if psi > crit_thresh:
                write_drift_event(session, feature_name, "judge_score_psi", psi, crit_thresh, "critical", len(cur_scores))
            elif psi > warn_thresh:
                write_drift_event(session, feature_name, "judge_score_psi", psi, warn_thresh, "warning", len(cur_scores))
...
if __name__ == "__main__":
    logger.info(f"Running drift detection scheduler at {datetime.utcnow().isoformat()}...")
