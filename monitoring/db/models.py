...
from sqlalchemy import JSON, ARRAY, Numeric, String, Text, DateTime, ForeignKey, Integer
...
class ModelConfig(Base):
    ...
    temperature: Mapped[Optional[float]] = mapped_column(Numeric, nullable=True)
    max_tokens: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    other_params: Mapped[Optional[dict[str, Any]]] = mapped_column(JSON, nullable=True)
    content_hash: Mapped[str] = mapped_column(Text)
...
class GoldenSetCase(Base):
    ...
    id: Mapped[UUID] = mapped_column(primary_key=True)
    golden_set_version_id: Mapped[UUID] = mapped_column(ForeignKey("golden_set_versions.id"))
    ...
    expected_criteria: Mapped[Optional[dict[str, Any]]] = mapped_column(JSON, nullable=True)
...
class RunTrace(Base):
    ...
    model_config_id: Mapped[UUID] = mapped_column(ForeignKey("model_configs.id"))
    golden_set_version_id: Mapped[Optional[UUID]] = mapped_column(ForeignKey("golden_set_versions.id"), nullable=True)
    input_payload: Mapped[dict[str, Any]] = mapped_column(JSON)
...
class JudgeVersion(Base):
    ...
    judge_model_config_id: Mapped[UUID] = mapped_column(ForeignKey("model_configs.id"))
    calibration_status: Mapped[str] = mapped_column(Text)
    last_kappa_score: Mapped[Optional[float]] = mapped_column(Numeric, nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class JudgeScore(Base):
    ...
    judge_version_id: Mapped[UUID] = mapped_column(ForeignKey("judge_versions.id"))
    dimension: Mapped[str] = mapped_column(Text)
    score: Mapped[float] = mapped_column(Numeric)
    rationale: Mapped[str] = mapped_column(Text)
    raw_judge_response: Mapped[Optional[dict[str, Any]]] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class RegressionRun(Base):
    ...
    baseline_prompt_version_id: Mapped[UUID] = mapped_column(ForeignKey("prompt_versions.id"))
    candidate_prompt_version_id: Mapped[UUID] = mapped_column(ForeignKey("prompt_versions.id"))
    baseline_model_config_id: Mapped[UUID] = mapped_column(ForeignKey("model_configs.id"))
    candidate_model_config_id: Mapped[UUID] = mapped_column(ForeignKey("model_configs.id"))
    golden_set_version_id: Mapped[UUID] = mapped_column(ForeignKey("golden_set_versions.id"))
    judge_version_id: Mapped[UUID] = mapped_column(ForeignKey("judge_versions.id"))
...
class RegressionCaseResult(Base):
    ...
    id: Mapped[UUID] = mapped_column(primary_key=True)
    regression_run_id: Mapped[UUID] = mapped_column(ForeignKey("regression_runs.id"))
    golden_set_case_id: Mapped[UUID] = mapped_column(ForeignKey("golden_set_cases.id"))
    baseline_run_trace_id: Mapped[UUID] = mapped_column(ForeignKey("run_traces.id"))
    candidate_run_trace_id: Mapped[UUID] = mapped_column(ForeignKey("run_traces.id"))
...
class DriftReferenceDistribution(Base):
    ...
    superseded_by: Mapped[Optional[UUID]] = mapped_column(ForeignKey("drift_reference_distributions.id"), nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class DriftEvent(Base):
    ...
    window_end: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True))
    reference_distribution_id: Mapped[UUID] = mapped_column(ForeignKey("drift_reference_distributions.id"))
