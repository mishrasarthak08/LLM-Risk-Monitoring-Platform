...
def run_feature_evaluation(prompt_path: str, input_payload: Dict[str, Any]) -> str:
    ...
    # We pass the input_payload to the model prompt directly in a real
    # implementation. Here we just inject the payload strings if needed,
    # but our templates don't take it right now.
    # We'll just pass the prompts object and it will run through the traced
    # LLM calls. To use the input payload properly, the credit_memo.py
    # prompts would be formatted with it.
    output = generate_credit_memo(
        prompts=prompts, model_config=model_config, input_payload=input_payload
    )
    return output
...
def run_regression_suite(
    ...
    for case in cases:
        ...
        for _ in range(repeats):
            ...
            base_scores = score_output(
                rubric, case.input_payload, base_out, case.expected_criteria
            )
            # Find factual accuracy score or default to 0
            base_fa = next(
                (d["score"] for d in base_scores
                 if d["dimension"] == "factual_accuracy" and d["score"] is not None),
                0
            )
            ...
            cand_scores = score_output(
                rubric, case.input_payload, cand_out, case.expected_criteria
            )
            cand_fa = next(
                (d["score"] for d in cand_scores
                 if d["dimension"] == "factual_accuracy" and d["score"] is not None),
                0
            )
        ...
        comp = compare_case(
            str(case.case_hash), case.severity, baseline_median, candidate_median
        )
